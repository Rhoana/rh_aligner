# Debugs the output of a matching sifts between two layers.
...
def call_java_debug_match_sifts(input_file, out_dir, jar_file):
    input_url = utils.path2url(os.path.abspath(input_file))
    java_cmd = 'java -Xmx4g -Djava.awt.headless=true -cp "{0}" org.janelia.alignment.DebugCorrespondence --inputfile {1} --targetDir {2} --sifts'.format(\
...
def get_matched_sifts_file(work_dir, s1_fname, s2_fname):
    # Returns the matched sifts file name
    matched_sifts_dir = os.path.join(work_dir, "matched_sifts")
...
def debug_match_sifts(tiles_dir, work_dir, out_dir, section_num1, section_num2, jar_file):
    fname1, fname2 = find_section_files(tiles_dir, section_num1, section_num2)
    if fname1 != None and fname2 != None:
        match_fname = get_matched_sifts_file(work_dir, fname1, fname2)
        call_java_debug_match_sifts(match_fname, out_dir, jar_file)



def main():
    # Command line parser
    parser = argparse.ArgumentParser(description='Debugs the output of a matching sifts between two layers.\
        Produces the images of the given sections (if a match output is found)')
    ...
    parser.add_argument('-o', '--output_dir', type=str, 
                        help='an output directory (default: ./matched_sifts_debug)',
                        default='./matched_sifts_debug')
    ...
    debug_match_sifts(args.tiles_dir, args.work_dir, args.output_dir, args.section_num1, args.section_num2, args.jar_file)
