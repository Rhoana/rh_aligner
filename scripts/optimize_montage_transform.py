...
def optimize_montage_transform(correspondence_file, tilespec_file, fixed_tiles, output_file, jar_file, conf_fname=None):
    ...
    java_cmd = 'java -Xmx4g -XX:ParallelGCThreads=1 -Djava.awt.headless=true -cp "{0}" org.janelia.alignment.OptimizeMontageTransform {1} --inputfile {2} --tilespecfile {3} {4} --targetPath {5}'.format(\
