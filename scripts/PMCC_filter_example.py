...
def PMCC_match(image, template, min_correlation=0.6, maximal_curvature_ratio=10, maximal_ROD=0.9):
...
if __name__ == '__main__':
    ...
    for i in range(100, image_1.shape[0], 100):
        for j in range(0, image_1.shape[1], 100):
            template = image_1[i:, j:][:200, :200].copy()
            result, location = PMCC_match(image_2, template, min_correlation=0.3)
            if result:
                print location
            else:
                print "     No match"
